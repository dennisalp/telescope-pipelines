...
tbin=4
tbpy=4
pn_src_prel="circle(17805.535,34487.674,400)"
m2_src_prel="circle(17805.535,34487.674,400)"
pn_bkg="circle(21283.439,34440.5,1525.8949)"
m2_bkg="annulus(17805.535,34487.674,1000,2000)"
...
# emllist_idx=1
# DET_ML=144.9162, (173.530961353948, 0.87353727737383)
# uncertainty (1.008628**2+1.2**2)**0.5/3600.=0.0004354409609015489=1.567587459245576 arcsec
coo="11:34:07.4307 +0:52:24.734"
coo_fit="circle(17827.695,34505.817,400)"

# optradius: 29 arcsecs 580 image units
# optradius: 28 arcsecs 560 image units
pn_src="circle(17827.695,34505.817,580)"
m2_src="circle(17827.695,34505.817,560)"
          
tpeak_min=566882362.294536
tpeak_max=566882393.70063
tpeak_bin=31.406100
...
# emosaic imagesets="${id}_pn_clean_img.fits ${id}_m2_clean_img.fits" \
...
# edetect_chain imagesets="${id}_pn_dirty_img_during.fits ${id}_m2_dirty_img_during.fits" \
...
# mk_evt during ${t_rise} ${t_fade} dirty_evt.fits
...
mk_spec_np () { # Same as pn but not flag==0
    evselect table=${id}_pn_clean_evt_${1}.fits withspectrumset=yes \
             spectrumset=${id}_np_spec_src_${1}.fits energycolumn=PI spectralbinsize=5 \
             withspecranges=yes specchannelmin=0 specchannelmax=20479 \
             expression="(X,Y) IN ${pn_src}"
    evselect table=${id}_pn_clean_evt_${1}.fits withspectrumset=yes \
             spectrumset=${id}_np_spec_bkg_${1}.fits energycolumn=PI spectralbinsize=5 \
             withspecranges=yes specchannelmin=0 specchannelmax=20479 \
             expression="(X,Y) IN ${pn_bkg}"
    backscale spectrumset=${id}_np_spec_src_${1}.fits badpixlocation=${id}_pn_clean_evt_${1}.fits
    backscale spectrumset=${id}_np_spec_bkg_${1}.fits badpixlocation=${id}_pn_clean_evt_${1}.fits
    rmfgen spectrumset=${id}_np_spec_src_${1}.fits rmfset=${id}_np_spec_rmf_${1}.fits
    arfgen spectrumset=${id}_np_spec_src_${1}.fits arfset=${id}_np_spec_arf_${1}.fits withrmfset=yes \
           rmfset=${id}_np_spec_rmf_${1}.fits badpixlocation=${id}_pn_clean_evt_${1}.fits \
           detmaptype=psf
    specgroup spectrumset=${id}_np_spec_src_${1}.fits mincounts=1 \
              rmfset=${id}_np_spec_rmf_${1}.fits arfset=${id}_np_spec_arf_${1}.fits \
              backgndset=${id}_np_spec_bkg_${1}.fits groupedset=${id}_np_spec_grp_${1}.fits
    fthedit "${id}_np_spec_src_${1}.fits" BACKFILE add "${id}_np_spec_bkg_${1}.fits"
    fthedit "${id}_np_spec_src_${1}.fits" RESPFILE add "${id}_np_spec_rmf_${1}.fits"
    fthedit "${id}_np_spec_src_${1}.fits" ANCRFILE add "${id}_np_spec_arf_${1}.fits"
}
