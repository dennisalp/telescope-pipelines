...
tbin=10
tbpy=4
pn_src_prel="circle(35299.442,25221.212,400)"
m1_src_prel="circle(35299.442,25221.212,400)"
m2_src_prel="circle(35299.442,25221.212,400)"
pn_bkg="circle(37126.912,24849.605,1148.5777)"
m1_bkg="circle(34876.391,27159.059,1342.4789)"
m2_bkg="circle(35860.198,22040.742,1476.7856)"
...
# emllist_idx=1
# DET_ML=808.4183, (24.2752046081828, -12.952798320756)
# uncertainty (0.6777027**2+1.2**2)**0.5/3600.=0.0003828177843134689=1.378144023528488 arcsec
coo="01:37:06.0491 -12:57:10.074"
coo_fit="circle(35309.166,25203.064,400)"

# optradius: 63 arcsecs 1260 image units (capped by nearby source to 900 image units)
# optradius: 43 arcsecs 860 image units
# optradius: 47 arcsecs 940 image units
pn_src="circle(35309.166,25203.064,900)"
m1_src="circle(35309.166,25203.064,860)"
m2_src="circle(35309.166,25203.064,900)"

# 51 counts within 12.741338 seconds between 298587897.430378 and 298587910.171715
tpeak_min=298587897.430378
tpeak_max=298587910.17171
tpeak_bin=12.741338
...
mk_evt () {
    ...
    evselect table=${id}_m1_${4} withfilteredset=Y \
...
mk_spec_ep () {    
    ...
    evselect table=${id}_m1_${2} withspectrumset=yes \
            spectrumset=${id}_m1_spec_src_tmp_${1}.fits energycolumn=PI spectralbinsize=10 \
            withspecranges=yes specchannelmin=0 specchannelmax=11999 \
            expression="(X,Y) IN ${m1_src}"
    evselect table=${id}_m1_${2} withspectrumset=yes \
            spectrumset=${id}_m1_spec_bkg_tmp_${1}.fits energycolumn=PI spectralbinsize=10 \
            withspecranges=yes specchannelmin=0 specchannelmax=11999 \
            expression="(X,Y) IN ${m1_bkg}"
...
mk_pup () {
    evselect table=$(ls *EPN*ImagingEvts.ds | head -${pn_exp} | tail -1) withfilteredset=Y \
             filteredset=${id}_pn_pup_evt_${1}.fits \
             destruct=Y keepfilteroutput=T \
             expression="(TIME>${2}) && (TIME<${3}) && ((X,Y) IN ${pn_src})"
    evselect table=$(ls *EMOS1*ImagingEvts.ds | head -${m1_exp} | tail -1) withfilteredset=Y \
             filteredset=${id}_m1_pup_evt_${1}.fits \
             destruct=Y keepfilteroutput=T \
             expression="(TIME>${2}) && (TIME<${3}) && ((X,Y) IN ${m1_src})"
    evselect table=$(ls *EMOS2*ImagingEvts.ds | head -${m2_exp} | tail -1) withfilteredset=Y \
             filteredset=${id}_m2_pup_evt_${1}.fits \
             destruct=Y keepfilteroutput=T \
             expression="(TIME>${2}) && (TIME<${3}) && ((X,Y) IN ${m2_src})"

    epatplot set=${id}_pn_pup_evt_${1}.fits plotfile=${id}_pn_pup_pat_${1}.ps
    epatplot set=${id}_m1_pup_evt_${1}.fits plotfile=${id}_m1_pup_pat_${1}.ps
    epatplot set=${id}_m2_pup_evt_${1}.fits plotfile=${id}_m2_pup_pat_${1}.ps
}
...
# edetect_chain imagesets="${id}_pn_clean_img.fits ${id}_m1_clean_img.fits ${id}_m2_clean_img.fits" \
#               eventsets="${id}_pn_clean_evt.fits ${id}_m1_clean_evt.fits ${id}_m2_clean_evt.fits" \
#               attitudeset=$(ls *AttHk.ds) \
#               pimin="300 300 300" \
#               pimax="10000 10000 10000" \
...
# evselect table=${id}_m1_dirty_evt.fits energycolumn=PI \
#         expression="(X,Y) IN ${m1_src}" \
#         withrateset=yes rateset=${id}_m1_raw_src_py_lc.fits timebinsize=${tbpy} \
#         maketimecolumn=yes makeratecolumn=yes timemin=${timemin} timemax=${timemax}
# evselect table=${id}_m1_dirty_evt.fits energycolumn=PI \
#         expression="(X,Y) IN ${m1_bkg}" \
#         withrateset=yes rateset=${id}_m1_raw_bkg_py_lc.fits timebinsize=${tbpy} \
#         maketimecolumn=yes makeratecolumn=yes timemin=${timemin} timemax=${timemax}
# epiclccorr srctslist=${id}_m1_raw_src_py_lc.fits eventlist=${id}_m1_dirty_evt.fits \
#           outset=${id}_m1_py_lccorr.fits bkgtslist=${id}_m1_raw_bkg_py_lc.fits \
#           withbkgset=yes applyabsolutecorrections=yes
