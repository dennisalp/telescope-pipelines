...
tbin=400
tbpy=4
pn_src_prel="circle(22146.593,26323.605,400)"
m1_src_prel="circle(22146.593,26323.605,400)"
m2_src_prel="circle(22146.593,26323.605,400)"
pn_bkg="circle(23606.330,27499.283,1227.3669)"
m1_bkg="circle(24361.187,23934.459,1405.1937)"
m2_bkg="circle(24361.187,23934.459,1405.1937)"
...
# emllist_idx=57
# DET_ML=75.10458, (43.6535581099581, 41.0742998235426)
# uncertainty (0.9510565**2+1.2**2)**0.5/3600.=0.0004253274397841651=1.5311787832229944 arcsec
coo="02:54:36.8539 +41:04:27.479"
coo_fit="circle(22136.493,26348.1,400)"

# optradius: 14 arcsecs 280 image units
# optradius: 14 arcsecs 280 image units
# optradius: 11 arcsecs 220 image units
pn_src="circle(22136.493,26348.1,280)"
m1_src="circle(22136.493,26348.1,280)"
m2_src="circle(22136.493,26348.1,220)"

# 26 counts within 806.873439 seconds between 524174986.304562 and 524175793.178001
tpeak_min=524174986.304562
tpeak_max=524175793.17800
tpeak_bin=806.873439
...
mk_evt () {
    ...
    evselect table=${id}_m1_clean_evt.fits withfilteredset=Y \
...
mk_img () {
    ...
    evselect table=${id}_m1_clean_evt_${1}.fits imagebinning=binSize \
...
mk_src_list () {
    edetect_chain imagesets="${id}_pn_clean_img_${1}.fits ${id}_m1_clean_img_${1}.fits ${id}_m2_clean_img_${1}.fits" \
                  eventsets="${id}_pn_clean_evt_${1}.fits ${id}_m1_clean_evt_${1}.fits ${id}_m2_clean_evt_${1}.fits" \
...
mk_spec () {
    ...
    # evselect table=${id}_m1_clean_evt_${1}.fits withspectrumset=yes \
    ...
    # backscale spectrumset=${id}_m1_spec_src_${1}.fits badpixlocation=${id}_m1_clean_evt_${1}.fits
    # backscale spectrumset=${id}_m1_spec_bkg_${1}.fits badpixlocation=${id}_m1_clean_evt_${1}.fits
    ...
    #       rmfset=${id}_m1_spec_rmf_${1}.fits badpixlocation=${id}_m1_clean_evt_${1}.fits \
    #       detmaptype=psf
    ...
    if [ ${1} = "before" ]; then
        tmp=$(fkeyprint ${id}_m2_spec_bkg_${1}.fits EXPOSURE | grep EXPOSURE | tail -1 | awk '{print $2}')
        echo EDITING
        echo $tmp
        fthedit "${id}_m2_spec_src_${1}.fits" EXPOSURE add ${tmp}
    fi
...
# evselect table=${id}_m1_clean_evt.fits energycolumn=PI \
...
#         maketimecolumn=yes makeratecolumn=yes timemin=${timemin} timemax=${timemax}
# epiclccorr srctslist=${id}_m1_raw_src_prel_lc.fits eventlist=${id}_m1_clean_evt.fits \
...
# evselect table=${id}_m1_clean_evt.fits withfilteredset=Y \
#         filteredset=${id}_m1_clean_evt_during.fits \
#         destruct=Y keepfilteroutput=T \
#         expression="(TIME>${t_rise}) && (TIME<${t_fade})"
# evselect table=${id}_m1_clean_evt_during.fits imagebinning=binSize \
#         imageset=${id}_m1_clean_img_during.fits withimageset=yes \
#         xcolumn=X ycolumn=Y ximagebinsize=80 yimagebinsize=80
...
# emosaic imagesets="${id}_pn_clean_img_during.fits ${id}_m1_clean_img_during.fits ${id}_m2_clean_img_during.fits" \
...
#                         ${id}_m1_clean_img_during.fits \
#                         ${id}_m2_clean_img_during.fits" \
#               eventsets="${id}_pn_clean_evt_during.fits ${id}_m1_clean_evt_during.fits ${id}_m2_clean_evt_during.fits" \
#               attitudeset=$(ls *AttHk.ds) \
#               pimin="300 300 300" \
#               pimax="10000 10000 10000" \
...
# esensmap detmasksets="${id}_pn_clean_img_duringmask.fits ${id}_m1_clean_img_duringmask.fits ${id}_m2_clean_img_duringmask.fits" \
#          expimagesets="${id}_pn_clean_img_duringexp.fits ${id}_m1_clean_img_duringexp.fits ${id}_m2_clean_img_duringexp.fits" \
#          bkgimagesets="${id}_pn_clean_img_duringbkg.fits ${id}_m1_clean_img_duringbkg.fits ${id}_m2_clean_img_duringbkg.fits" \
...
# eregionanalyse imageset=${id}_m1_clean_img_during.fits \
#                srcexp="(X,Y) in ${coo_fit}" \
...
#         maketimecolumn=yes makeratecolumn=yes timemin=${timemin} timemax=${timemax}
# epiclccorr srctslist=${id}_m1_raw_src_lc.fits eventlist=${id}_m1_clean_evt.fits \
...
evselect table=${id}_m1_clean_evt.fits energycolumn=PI \
        ...
        maketimecolumn=yes makeratecolumn=yes timemin=${timemin} timemax=${timemax}
epiclccorr srctslist=${id}_m1_raw_src_py_lc.fits eventlist=${id}_m1_clean_evt.fits \
...
#         maketimecolumn=yes makeratecolumn=yes timemin=${tpeak_min} timemax=${tpeak_max}
# epiclccorr srctslist=${id}_m1_raw_src_lc_peak.fits eventlist=${id}_m1_clean_evt.fits \
